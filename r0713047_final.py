...
def eliminitaion(population: np.ndarray, fitness_values: np.ndarray, sigma_: np.float64, alpha_: np.float64, lambda_: np.int64, elimination_k_: np.int64) -> np.ndarray:
    ...
    def update_shared_fitnesses(shared_fitnesses: np.ndarray, fitnesses: np.ndarray, penalties: np.ndarray, winner: np.ndarray):
        random_sample_indices = np.random.choice(pop_indices[mask], pop_indices[mask].shape[0]//7, replace=False)
    ...
    for i in np.arange(1, lambda_):
        ...
        # k tournament selection based on fitness values
...
class r0713047:
    ...
    def optimize(self, filename):
        ...
        mu_                       = np.int64(220)
        selection_k_              = np.int64(lambda_//40)          
        elimination_k_            = np.int64((lambda_ + mu_)//40)  
        off_mutation_alpha_       = np.float64(0.20)               
        pop_mutation_alpha_       = np.float64(0.2)               
        num_elites_               = np.int64(1)
        fitness_sharing_sigma_    = np.float64(0.1*num_cities)     
